...
export default function OwnerOrdersPage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : t.orders.failedToLoadProfile
        );
        setLoading(false);
      }
    };
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t.orders.errorLoadingOrders);
    } finally {
      setLoading(false);
    }
  }, [cafeId, t.orders.errorLoadingOrders]);
  ...
  const handleStatusUpdate = async (orderId: string, newStatus: string) => {
    ...
    try {
      ...
      alert(t.orders.failedToUpdateStatus);
      console.error(err);
    } finally {
      setUpdatingId(null);
    }
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case "preparing":
        return {
          bg: "rgba(58,124,200,0.12)",
          color: "#2A6CB8",
          label: t.orders.status.preparing,
        };
      case "completed":
        return {
          bg: "rgba(40,160,90,0.12)",
          color: "#1A8A50",
      ...
      <div className="orders-root">
        <div className="page-header">
          ...
          <div className="refresh-info">
            <span className={`last-updated ${t.meta.isKhmer ? "khmer-text" : ""}`}>
              {t.orders.updatedAt}{" "}
            ...
            <button className="refresh-btn" onClick={() => void fetchOrders()}>
              ↻ {t.orders.refresh}
            </button>
          </div>
        </div>
        ...
        {!loading && (
          <div className="kpi-strip">
            <div className="kpi-card">
              <p className="kpi-value" style={{ color: "#B87800" }}>
                {pendingCount}
              </p>
              <p className={`kpi-label ${t.meta.isKhmer ? "khmer-text" : ""}`}>
                {t.orders.kpi.newOrders}
              </p>
            </div>

            <div className="kpi-card">
              <p className="kpi-value" style={{ color: "#2A6CB8" }}>
                {preparingCount}
              </p>
              <p className={`kpi-label ${t.meta.isKhmer ? "khmer-text" : ""}`}>
                {t.orders.kpi.preparing}
              </p>
            </div>

            <div className="kpi-card">
              <p className="kpi-value" style={{ color: "#C8873A" }}>
                ${todayRevenue.toFixed(2)}
              </p>
              <p className={`kpi-label ${t.meta.isKhmer ? "khmer-text" : ""}`}>
                {t.orders.kpi.collected}
              </p>
            </div>
        ...
        {pendingCount > 0 && (
          <div className={`new-orders-alert ${t.meta.isKhmer ? "khmer-text" : ""}`}>
            <span className="alert-dot" />
            {t.orders.pendingAlert(pendingCount)}
          </div>
        )}

        <div className="filters">
          {[
            { key: "active", label: `${t.orders.filters.active} (${activeOrders.length})` },
            {
              key: "completed",
              label: `${t.orders.filters.completed} (${completedOrders.length})`,
            },
              ...
              <div key={order.id} className={`order-card ${order.status}`}>
                <div className="order-top">
                  <div>
                    ...
                    <p className={`order-time ${t.meta.isKhmer ? "khmer-text" : ""}`}>
                ...
                <div className="order-items">
                  {order.order_items.map((item) => (
                    <div key={item.id} className="item-chip">
                      ...
                      <span>{item.menus?.name || t.orders.unknownItem}</span>
                      {item.notes && (
                        <span className={`item-notes ${t.meta.isKhmer ? "khmer-text" : ""}`}>
                          — {item.notes}
                        </span>
                      )}
                    </div>
                ...
                <div className="order-actions">
                  ...
                  {order.status === "completed" && (
                    <span className={`action-btn done-label ${t.meta.isKhmer ? "khmer-text" : ""}`}>
                      {t.orders.actions.completedPaid}
                    </span>
                  )}
                </div>
              </div>
